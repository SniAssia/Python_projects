...
# q1 : print gender of the student who has the lowest average in all tps
...
# q2 : names of students who were absent in the first session
l=[]
...
print('absent students in the first session  are : ',absent)

# q3 : names of students who have improved their exam scores over the first three
# exams
...
# q4 : the grp of students who have at least one tp score above 18
...
# q5 : students names that have attendance rate higher than 0.95
l9=[]
...
# q6 : calculate the mean exam mark for each student , then print the name of the
# top student for each grp
...
# q7 : for each grp print the mean mark of tps for female students
# this function will count number of females in each grp
def grp(group,var):
    c=0
...
# q8 : mean mark of tps for each gender in the grp with the most
# students
fem=cal_aver(data['TPs'])
...
# q9 : minimum tp marks for each gender
...
# q10 : average exam score for all students for each gender
...
# q11 : for each session , print the name of students who were absent
...
# q12 : for male students , print the average exam mark for each grp
...
# q13 : student's name with the highest mark in the last exam for each grp
print(data['Exams'][-1])
...
print(dict_high)
"""
# q14 : for each gender , identify the grp of student with the highest final_mark
...
matrix = []
for day in data['Attendance']:
    matrix.append(data['Attendance'][day][1])
mat43=calculate_mean(matrix)
print(mat43)
l2=[]
for i in data['min_att']:
    print(i,data['min_att'][i],data['dic_atd'][i])
for i in range(len(mat43)):
    if 'Present' in data['min_att'] :
        if mat43[i] in data['min_att']['Present']:
            l2.append(data['dic_atd']['Present'])
    elif 'Late' in data['min_att'] :
        if mat43[i] in data['min_att']['Late']:
            l2.append(data['dic_atd']['Late'])
    elif 'Very Late' in data['min_att'] :
        if mat43[i] in data['min_att']['Very Late']:
            l2.append(data['dic_atd']['Very Late'])
    elif 'Absent' in data['min_att'] :
        if mat43[i] in data['min_att']['Absent']:
            l2.append(data['dic_atd']['Absent'])
final_marks = []
output={'Male': [],'Female': []}
for i in range(len(data['Student'])):
    final_mark=(calculate_mean(data["Exams"])[i]*0.6+calculate_mean(data["TPs"])[i]*0.15+calculate_median(data["Exams"][i])*0.25)*l2[i]
    final_marks.append(final_mark)
male = []
female = []
for i in range(len(final_marks)):
    if data['Gender'][i] == "Male":
        male.append(final_marks[i])
    elif data['Gender'][i] == "Female":
        female.append(final_marks[i])
max_index_m=[index for index,i in enumerate(male) if i==max(male)]
max_index_f = [index for index, i in enumerate(female) if i == max(female)]
for j in max_index_m:
    output['Male'].append(data['Group'][final_marks.index(male[j])])
for i in max_index_f:
    output['Female'].append(data['Group'][final_marks.index(male[i])])

print(output)

"""
# q15 : for each grp , print the gender of the student with the highest attendance
# duration in all the sessions
dict_dura = {x: {y:[] for y in GROUPS} for x in data['Attendance']}
for day in data['Attendance']:
    for k in range(0 ,4):
        for i in range(len(data['Attendance'][day][1])):
            if data['Group'][i]==GROUPS[k] :
                l.append(data['Attendance'][day][1][i])
        max_index=[index for index ,x in enumerate(l) if x==max(l)]
        for j in max_index:
            dict_dura[day][GROUPS[k]].append(data['Gender'][data['Attendance'][day][1].index(l[j])])

print(dict_dura)

# q16 : print the session with the most absences
absences = []
days = []
final_list = []
for day in data['Attendance']:
...
if len(final_list)==1:
    ...
    print(final_list)
# q17 : session with the most delays
days = []
for day in data['Attendance']:
    days.append(day)
max_l = []
...
# q18 : for each student , print the number of exams where they scored above their group's
# average exam score
def grp1(group):
    c=0
    for i in range(len(data['Group'])):
...
# q19 : for each gender , print the average delay across all sessions
def grp2(var):
    c=0
...
# q20 : for each gender , print the number of the students with average tp score above 12
...
class student:
    ...
    def add_subject(self, subject, assessement_type, grade):
        if subject not in self.subject_grades:
            self.subject_grades[subject] = []
        self.subject_grades[subject].append(assessement(assessement_type, grade))
    ...
    def print_info(self):
        print("Name: ", self.name, ", Gender: ", self.gender, ", Group: ", self.group)
        print("\nSubject Grades:")
        for subject, assessments in self.subject_grades.items():
            print("\nSubject: ", subject)
            for assessment in assessments:
                print("ID: ", assessment.ID, ", Type: ", assessment.type, ", Grade: ", assessment.grade)
        print("\nAttendance:")
        for subject, attendances in self.attendance.items():
            print("\nSubject: ", subject)
            print("Day of Attendance | Attendance Status | Delay in Minutes")
            for attendance in attendances:
                status = attendance.get_attendance_status()
                print(attendance.days, " | ", status, " | ", attendance.late_minutes)
